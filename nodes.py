...
class GrokZImageTurboPromptBatch(GrokSDXLPromptBatch):
    ...
    def _build_system_prompt(self, lora_mode, relevant, lora_indexer, seed_style):
        parts = [
            ...
            "2. AESTHETIC BOOSTERS (MANDATORY): Begin every prompt with powerful quality tags: 'masterpiece, best quality, ultra-detailed, 8k resolution, highly detailed face, realistic skin texture, perfect anatomy'.",
            "3. EXCLUSION TAGS (MANDATORY): End every prompt with this exact blocking string to prevent deformities: '(deformed, distorted, disfigured, poorly drawn, bad anatomy, wrong anatomy, extra limb, missing limb, floating limbs, mutated hands, extra fingers, text, watermark, logo:1.3)'.",
            "4. Be specific and detailed. Describe the scene like a movie director. Do not use vague words like 'nice' or 'beautiful'.",
            "5. Structure: Aesthetic Boosters -> Main Subject + Action -> Setting & Environment -> Lighting & Mood -> Style -> Exclusion Tags.",
            "6. NO token spam. Keep descriptions precise (80 to 200 words max).",
...
class GrokZImageTurboPromptBatchIdentical(GrokSDXLPromptBatchIdentical):
    ...
    def _build_system_prompt(self, lora_mode, relevant, lora_indexer, seed_style):
        parts = [
            ...
            "2. AESTHETIC BOOSTERS (MANDATORY): Begin every prompt with powerful quality tags: 'masterpiece, best quality, ultra-detailed, 8k resolution, highly detailed face, realistic skin texture, perfect anatomy'.",
            "3. EXCLUSION TAGS (MANDATORY): End every prompt with this exact blocking string to prevent deformities: '(deformed, distorted, disfigured, poorly drawn, bad anatomy, wrong anatomy, extra limb, missing limb, floating limbs, mutated hands, extra fingers, text, watermark, logo:1.3)'.",
            "4. Structure: Aesthetic Boosters -> Main Subject + Action -> Setting & Environment -> Lighting & Mood -> Style -> Exclusion Tags.",
            "5. Output ONLY the prompts, no numbering, no explanations.",
            "",
            "SUBJECT UNIFORMITY (CRITICAL & MANDATORY):",
            "1. When generating batches, you absolutely MUST make all prompts almost completely IDENTICAL.",
            "2. Do NOT randomly vary hair color, ethnicity, body type, age, clothing, or background.",
